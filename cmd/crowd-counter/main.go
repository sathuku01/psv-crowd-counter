...
func main() {
	...
	// Video path
	// videoPath := filepath.Join(root, "internal", "camera", "mock", "sample.mp4")
	// YOLOv8 ONNX path
	modelPath := filepath.Join(root, "internal", "core", "models", "yolov8n.onnx")
	...
	for {
		...
		defer blob.Close()
		// Debug blob for first frame
		if frameCount == 1 {
			fmt.Printf("Blob size: %v, type: %d\n", blob.Size(), blob.Type())
		}
		net.SetInput(blob, "")
		// Use Forward to get output (YOLOv8 typically has one main output)
		output := net.Forward("")

		if output.Empty() {
			log.Printf("Warning: empty output from network")
			output.Close()
			continue
		}
		...
		if len(data) == 0 {
			log.Printf("Warning: output data is empty")
			output.Close()
			continue
		}
		...
		size := output.Size()
		if len(size) == 0 {
			log.Printf("Warning: output shape is empty")
			output.Close()
			continue
		}
		// Check if any dimension is invalid (<=0)
		for _, d := range size {
			if d <= 0 {
				log.Printf("Warning: invalid output shape dimension: %v", size)
				output.Close()
				continue
			}
		}
		...
		// - 2D: [num_predictions, num_classes+4] (e.g., [8400, 84])
		// - 3D (old): [1, num_classes+4, num_predictions] (e.g., [1, 84, 8400])
		// - 3D (new): [1, num_predictions, num_classes+4] (e.g., [1, 8400, 84])
		if len(size) == 3 {
			// 3D output format
			var numPredictions, numChannels int

			// Determine format based on shape values
			// Check if middle dimension is small (likely num_classes+4)
			if size[1] < 100 {
				// Format: [1, num_classes+4, num_predictions]
				numPredictions = size[2]
				numChannels = size[1]
			} else {
				// Format: [1, num_predictions, num_classes+4]
			...
			// 2D output format: [num_predictions, num_channels]
			numPredictions := size[0]
			numChannels := size[1]

			if frameCount == 1 {
				fmt.Printf("Parsing YOLOv8 2D output: %d predictions, %d channels\n",
					numPredictions, numChannels)
			}
			...
			log.Printf("Warning: unexpected output shape dimensions: %d", len(size))
		}
